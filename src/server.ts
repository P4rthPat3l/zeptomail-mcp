...
import { ZeptoMailApiError, ZeptoMailClient, type TokenProvider } from './zeptomail-client.js';
import { PerClientZohoTokenProvider, ZohoOAuthTokenProvider } from './zoho-oauth.js';
...
function requireWritePermission(config: Config, confirmed: boolean, action: string): void {
...
export function buildServer(config: Config, tokenProvider: TokenProvider): McpServer {
  const client = new ZeptoMailClient(config, tokenProvider);
  const server = new McpServer({ name: 'zeptomail-templates', version: '0.3.0' });
  ...
  server.registerTool(
    ...
    async ({ agentKey, offset, limit }) => {
      try {
        ...
        return { content: [{ type: 'text', text: asText({ agent, ...result }) }] };
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  server.registerTool(
    ...
    async ({ agentKey, templateKey }) => {
      try {
        ...
        return { content: [{ type: 'text', text: asText({ agent, template }) }] };
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  server.registerTool(
    ...
    async ({ agentKey, expectedAgentName, confirm, ...input }) => {
      try {
        ...
        const result = await client.createTemplate(agentKey, expectedAgentName, {
          templateName: input.templateName,
          subject: input.subject,
          ...(input.templateAlias ? { templateAlias: input.templateAlias } : {}),
          ...(input.htmlBody ? { htmlBody: input.htmlBody } : {}),
          ...(input.textBody ? { textBody: input.textBody } : {}),
        });
        return {
          content: [{ type: 'text', text: asText({ agentKey, expectedAgentName, result }) }],
        };
      } catch (error) {
        return toolError(error);
      }
  ...
  server.registerTool(
    ...
    async ({ agentKey, expectedAgentName, templateKey, confirm, ...input }) => {
      try {
        ...
        const result = await client.updateTemplate(agentKey, templateKey, expectedAgentName, {
...
function startHttp(config: Config): void {
  ...
  app.listen(config.port, () => {
    console.error(`ZeptoMail MCP server listening on ${config.serverUrl} (OAuth enabled)`);
  });
}
